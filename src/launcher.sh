...
# Reads macOS SystemConfiguration on every start and never changes launchctl
# environment or the official Codex.app bundle.
...
elif [[ "${1:-}" == '--test-launch' ]]; then
  script_mode='test-launch'
fi

codex_app_override=''
test_candidates_root=''
system_scutil='/usr/sbin/scutil'
system_nc='/usr/bin/nc'
system_mdfind='/usr/bin/mdfind'
if [[ "$script_mode" == 'self-test' || "$script_mode" == 'test-launch' ]]; then
  codex_app_override="${CODEX_BRIDGE_TEST_CODEX_APP:-}"
  test_candidates_root="${CODEX_BRIDGE_TEST_CANDIDATES_ROOT:-}"
  system_scutil="${CODEX_BRIDGE_TEST_SCUTIL:-$system_scutil}"
  system_nc="${CODEX_BRIDGE_TEST_NC:-$system_nc}"
  system_mdfind="${CODEX_BRIDGE_TEST_MDFIND:-$system_mdfind}"
fi

candidate_path() {
  local candidate="$1"
  if [[ -n "$test_candidates_root" ]]; then
    print -r -- "$test_candidates_root$candidate"
  else
    print -r -- "$candidate"
  fi
}
...
find_codex_app() {
  ...
  if [[ -n "$codex_app_override" ]]; then
    is_codex_bundle "$codex_app_override" && print -r -- "$codex_app_override" && return
  fi

  for candidate in \
    "$(candidate_path '/Applications/ChatGPT.app')" \
    "$(candidate_path '/Applications/Codex.app')" \
    "$(candidate_path "$HOME/Applications/ChatGPT.app")" \
    "$(candidate_path "$HOME/Applications/Codex.app")"; do
  ...
  done < <("$system_mdfind" 'kMDItemCFBundleIdentifier == "com.openai.codex"c' 2>/dev/null)
}

notify() {
  local message="$1"
  /usr/bin/osascript -e "display notification \"${message//\"/\\\"}\" with title \"Clash Bridge\"" >/dev/null 2>&1 || true
}
...
if [[ -z "$codex_app" || ! -d "$codex_app" || ! -f "$codex_info" ]]; then
  if [[ "$script_mode" == 'self-test' || "$script_mode" == 'test-launch' ]]; then
    ...
    notify 'Official Codex bundle was not found; Codex was not started.'
  fi
  exit 1
fi

codex_bundle_id=$(/usr/libexec/PlistBuddy -c 'Print :CFBundleIdentifier' "$codex_info" 2>/dev/null || true)
if [[ "$codex_bundle_id" != 'com.openai.codex' ]]; then
  if [[ "$script_mode" == 'self-test' || "$script_mode" == 'test-launch' ]]; then
    print 'codex_bundle=wrong_bundle_id'
    print 'decision=fail_wrong_bundle_id'
  else
    notify 'The selected app is not the official Codex-mode bundle; Codex was not started.'
  fi
  exit 1
fi
...
if [[ -z "$codex_bundle_executable" || "$codex_bundle_executable" == *'/'* ]]; then
  if [[ "$script_mode" == 'self-test' || "$script_mode" == 'test-launch' ]]; then
    ...
    notify 'The Codex-mode app has no usable CFBundleExecutable; Codex was not started.'
  fi
  exit 1
fi
...
if [[ ! -x "$codex_executable" ]]; then
  if [[ "$script_mode" == 'self-test' || "$script_mode" == 'test-launch' ]]; then
    ...
    notify 'The Codex-mode app executable is unavailable; Codex was not started.'
  fi
  exit 1
fi
...
if [[ "$script_mode" == 'self-test' ]]; then
  print 'codex_bundle=present cf_bundle_id=verified cf_bundle_executable=read official_executable=executable'
  ...
  if [[ "$http_enabled" == '1' && "$https_enabled" == '1' ]] && valid_host "$http_host" && valid_host "$https_host" && valid_port "$http_port" && valid_port "$https_port"; then
    ...
    print "http_url=$(proxy_url "$http_host" "$http_port")"
    print "https_url=$(proxy_url "$https_host" "$https_port")"
...
if [[ "$http_enabled" == '1' && "$https_enabled" == '1' ]] && valid_host "$http_host" && valid_host "$https_host" && valid_port "$http_port" && valid_port "$https_port"; then
  ...
  export HTTP_PROXY="$(proxy_url "$http_host" "$http_port")"
  export HTTPS_PROXY="$(proxy_url "$https_host" "$https_port")"
...
if [[ "$script_mode" == 'test-launch' ]]; then
  print 'decision=launch_without_explicit_proxy'
fi
